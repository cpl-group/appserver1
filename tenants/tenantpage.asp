...
<tr bgcolor="#3399cc" class="standardheader">
    <td><font size="3">Available Bills for <%=tenantname%> (<%=tenantnum%>)</font></td>
  </tr>
</table>
...
if leaseid <> "" then
  ...
  <input type="button" name="Button" value="View Invoice" onclick="loadinvoice(ypid.value,lid.value)">
	...
	<iframe name="summary" src="https://appserver1.genergy.com/genergy2/UMreports/tenantBillSummary.asp?pid=<%=pid%>&building=<%=bldg%>&billingid=<%=lastypid%>&leaseutilityid=<%=leaseid%>&syear=<%=byear%>&utilityid=<%=utility%>" width="100%" height="65%" scrolling="auto" marginwidth="8" marginheight="16" style="border:none;">
	</iframe>
    <IFRAME name="invoice" src="http://pdfmaker.genergyonline.com<%=getKeyValue("billlink")%>genergy2=true&devIP=<%=request.ServerVariables("SERVER_NAME")%>&building=<%=bldg%>&lid=<%=leaseid%>&byear=<%=byear%>&bperiod=<%=bperiod%>&y=<%=lastypid%>&ypid=<%=lastypid%>&l=<%=leaseid%>&detailed=false&pid=<%=pid%>&utilityid=<%=utility%>" width="100%" height="25%" scrolling="auto" marginwidth="8" marginheight="16" style="border:none;"></IFRAME>
  <%else
