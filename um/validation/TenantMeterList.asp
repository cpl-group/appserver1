...
dim CurrentOutput, prevOutput, nometerdata
CurrentOutput = ""
prevOutput = ""
nometerdata = ""
...
if checking="Tenant" then
	...
	loop
	rst1.close
	rst1.open "select meternum from tblmetersbyperiod where meterid not in (SELECT c.meterid FROM consumption c, peakdemand p WHERE p.meterid=c.meterid and c.billperiod=p.billperiod and c.billyear=p.billyear and c.billyear="&byear&" and c.billperiod="&bperiod&") and BldgNum='"&building&"' and billyear="&byear&" and billperiod="&bperiod, cnn1
	do until rst1.eof
		nometerdata = nometerdata & rst1("meternum")&"<BR>"
		rst1.movenext
	loop
end if
...
if trim(nometerdata)<>"" then
	response.write "<b>Meters missing data from either consumption or peak demand.</b><br>"
	response.write nometerdata
end if
if trim(currentOutput)="" and trim(prevOutput)="" and trim(nometerdata)="" then
