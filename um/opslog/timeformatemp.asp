...
sql2 = "SELECT master_job.Description as description, Times.jobno, sum(Times.Hours) as hours, sum(Times.OverT) as overt, sum(Times.Value) as [Expense Value],case when master_job.id > 6283 then left(type,2)+'-00'+convert(varchar(4),master_job.id) else '00-00'+convert(varchar(4),master_job.id) end as  tjob FROM master_job  INNER JOIN Times ON master_job.id = Times.JobNo WHERE (Times.Date BETWEEN '"&startweek&"' AND '"&endweek&"') and times.matricola='"&user&"' group by master_job.Description,Times.JobNo,master_job.id,master_job.type"
