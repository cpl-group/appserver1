...
function rfpopen(frm){
    var temp = "newcustomer.asp?mkid="+ frm.id1.value +"&cust="+ frm.cust.value
...
sqlstr = "select contact as cust,* from mktlog m join contacts c on m.contact=c.id where  m.id=" & mkid

else
sqlstr="select c.id as cust,c.first_name+c.last_name as name,c.title as title,c.phone as phone,c.fax as fax,c.email as email,c.referredby,c.otherref as otherref,m.* from mktlog m,contacts c  where m.id=" & mkid&" and c.id="&cust
end if
