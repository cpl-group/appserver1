...
sqlstr = "select times.*, employees.category from times join employees on employees.username = times.matricola, master_job where jobno=master_job.id and times.entry_time > master_job.last_invoice and jobno='" & job &"' order by employees.category desc, date desc" 
