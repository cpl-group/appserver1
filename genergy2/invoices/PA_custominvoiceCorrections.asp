...
<table width="690" border="0" cellpadding="0" cellspacing="0" ID="Table2">
	<tr><td height="50" valign="middle">
      <font color="#000000"><h3>Utility Invoice - <%=utilityname%></h3></font>
    </td>
  </tr>
  <% 
	if (invoiceNum = "") then
	    sql = "SELECT bp.id as billid " & _
			    ...
			    " WHERE reject=0 and bp.ypid="&ypid&" and bp.leaseutilityid="&leaseid & " and bldgnum='" & building & "'" 
  ...
  <tr>
    <td height="200"><table width="680" height="170" border="0" cellpadding="0" cellspacing="2" ID="Table7">
      ...
      <tr>
		...
		<%
		'reset billid to the original building
		dim billidOrig, rst666, sql6
		Set rst666 = Server.CreateObject("ADODB.recordset")
		if building="JFKCORR" then
			sql6 = "SELECT bp.id as billid " & _
			    "FROM tblbillbyPeriod bp " & _
			    " WHERE reject=0 and bp.ypid="&ypid&" and bp.leaseutilityid="&leaseid & " and bldgnum='JFK'" 		
	    rst666.open sql6, getConnect(pid,building,"billing")
	    if not rst666.eof then
  		    billidOrig = rst666("billid")
  	    end if	
	    rst666.close
		end if
		...
		if extusage = "" then 
	     sql = "select extusg from tblbillbyperiod where id = " & billidOrig
		...
		meterinfo.open "select * from tblmetersbyperiod tm,buildings b,meters m where tm.bldgnum =b.bldgnum and tm.meterid=m.meterid and b.bldgnum = m.bldgnum and bill_id="&billidOrig&" order by tm.meternum", cnn1
	...
	<%
		if metercnt > 5  or (building="BATH" and utilityid=2 and byear=2010 and bperiod=3) or ((building="LGA" or building="JFK" or building="PABT") and utilityid=2 and byear=2010 and bperiod=4)then
			%><WxPrinter PageBreak><%
		end if
	%>
    </td>
          ...
          <tr>
            <td>
                ...
                <tr><td><%if trim(energydetail)<>"" then%><%if ucase(trim(serviceclass)) <> "SC4 RA1" and ucase(trim(serviceclass)) <> "SC4 RA2" then%>Consumption<%else%>Invoice<%end if%> Details:<%end if%></td>
	                <td><%if trim(demanddetail)<>"" then%>Demand Details:<%end if%></td></tr>
