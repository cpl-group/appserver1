...
if (trim(utilityname) = "Cold Water" or trim(utilityname) = "Hot Water") And (pid = 171) And (trim(bperiod) = 4) Then
    utilityname = "One time 6 week Water reading"
else
...
<tr><td><%if trim(rst2("energydetail"))<>"" then%><%if trim(serviceclass) = "ConEd SC9R1" then%>Supply Charge<%else if ucase(trim(serviceclass)) <> "SC4 RA1" and ucase(trim(serviceclass)) <> "SC4 RA2" then%>Consumption<%else%>Invoice<%end if%><%end if%> Details:<%end if%></td>
	<td><%if trim(rst2("demanddetail"))<>"" then%><%if trim(serviceclass) = "ConEd SC9R1" then%>Delivery Charge<%else%>Demand<%end if%> Details:<%end if%></td>
</tr>
...
<tr><td valign="top">
<%footer rst2("tenantname"), rst2("tenantnum"), rst2("billingaddress"), rst2("tcity"), rst2("tstate"), rst2("tzip"), rst2("btbldgname"), rst2("billingaddress"), rst2("btcity"), rst2("btstate"), rst2("billzip"), rst2("onlinebill"), textheader,demo, paymentterm, rst2("bldgnum")%>
</td></tr>
</table>
<%
...
'###########################################################################################
sub footer(tenantname, tenantnum, tstrt, tcity, tstate, tzip, btbldgname, billingaddress, btcity, btstate, btzip, isonlinebill,textheader,demo, paymentterm, bldgnum)%>
...
<%
	...
	%>
		<td width="50%" valign="top"><font size="1">Tenant Name and Address:</FONT><br>
	  <font size="3"><b><%=tenantname%> <% if tenantnum ="" or bldgnum = "rneembka" then %> <%= "" %> <% else %> (<%=tenantnum%>) <% end if %> <br>
