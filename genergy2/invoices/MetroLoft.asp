...
case 3, 10,6,1,4,21,22
	...
	sql = "SELECT isnull(b.btzip,'') as billzip, b.portfolioid, isnull(b.btstrt,'') as billto, r.addonfee as myaddonfee, r.taxexempt,isnull(r.energydetail,'0') as energydetail, isnull(r.demanddetail,'0') as demanddetail, r.utility as unittype, isnull(Totalamt,0) as Totalamtnonull, isnull(tax,0) as taxnonull, isnull(energy,0) as energynonull, isnull(r.tstrt,'') as billingaddress,isnull(r.adminfee,0) as adminfee,isnull(r.servicefee,0) as servicefeenonull, r.fueladj as fadj, isnull(demand,0) as demandnonull, isnull(credit,0) as creditnonull, isnull(r.adjustment, 0) as adjustmentnonull, isnull(credit,0) as credit, rt.[type] as rt,rt.[id] as rtid, datediff(day, datestart,dateend)+1 as days, (select case count(distinct isnull(addonfee,0)) when 0 then 0 else 1 end as aoncnt, taxexempt from tblmetersbyperiod where bill_id=r.id group by leaseutilityid,ypid,bill_id) as showaddonfee, r.invoice_note as invoiceNote, isnull(rate_servicefee_dollar,0) as rateservicefee_dollar, r.*, l.onlinebill FROM tblbillbyperiod r, tblleases l, buildings b, ratetypes rt WHERE r.ratetenant=rt.id AND b.bldgnum=r.bldgnum and l.billingid = (SELECT billingid FROM tblleasesutilityprices lup WHERE lup.leaseutilityid=r.leaseutilityid) and r.id="&billid
...
<%case else
  ...
  %>
  	<td align="right" colspan=2>Energy Supply Charges</td>
  	<td width="5">&nbsp;</td>
  	<td align="right"><%=FormatCurrency(rst2("energynonull"),2)%></td>
  </tr>
	  <%if SJPproperties then%>
		<tr>
  ...
  END IF
  dim energysalestax
  if rst2("taxexempt") = 0 then
     energysalestax = cdbl(rst2("energynonull")) * cDbl(.08875)
  else
     energysalestax = 0
  end if
  ...
  %>
  <tr>
  	<td align="right" colspan=2>Sales Tax(Supply)</td>
  	<td width="20">&nbsp;</td>
  	<td align="right"><%=FormatCurrency(energysalestax,2)%></td>
  </tr>
  <%
  dim energycostsubtotal
  energycostsubtotal = cDbl(rst2("energynonull")) + cDbl(energysalestax)%>
  <tr>
  	<td align="right" colspan=2><b>Total Energy Supply Charges</b></td>
  	<td width="5">&nbsp;</td>
  	<td align="right"><b><%=FormatCurrency(energycostsubtotal,2)%></b></td>
  </tr>
  <tr>
  	<td align="right" colspan=2>Gross Utility Company T&D Charges</td>
  	<td width="0">&nbsp;</td>
  	<td align="right"><%=FormatCurrency(rst2("demandnonull"),2)%></td>
  </tr>
  <%
  dim nettdcharges
  nettdcharges = 0
  IF UTILITYID <> 4 THEN 
   if isnumeric(rst2("unit_credit")) and trim(rst2("unit_credit"))<>"0" then
	  miscCredits 3, leaseid,billid
	  nettdcharges = cDbl(rst2("demandnonull")) - cDbl(creditamount)%>
	  <tr>
  	    <td align="right" colspan=2>Net Utility Company T&D Charges</td>
  	    <td width="5">&nbsp;</td>
  	    <td align="right"><%=FormatCurrency(nettdcharges,2)%></td>
      </tr><%
	end if
  END IF
  %>
   ...
   <%
  dim tdsalestax
  if nettdcharges > 0 then
     tdsalestax = cdbl(nettdcharges) * cDbl(.045)
  else
     tdsalestax = cdbl(rst2("demandnonull")) * cDbl(.045)
  end if
  	
  %>
  <tr>
  	<td align="right" colspan=2>Sales Tax(T&D)</td>
  	<td width="20">&nbsp;</td>
  	<td align="right"><%=FormatCurrency(tdsalestax,2)%></td>
  </tr>
  
  <%
  dim tdcostsubtotal
  if nettdcharges > 0 then
     tdcostsubtotal = cDbl(nettdcharges) + cDbl(tdsalestax)
  else
     tdcostsubtotal = cDbl(rst2("demandnonull")) + cDbl(tdsalestax)
  end if%>
   
  <tr>
  	<td align="right" colspan=2><b>Total Energy T&D Charges</b></td>
  	<td width="5">&nbsp;</td>
  	<td align="right"><b><%=FormatCurrency(tdcostsubtotal,2)%></b></td>
  </tr>
  
   <%
  dim costsubtotal
  costsubtotal = round(energycostsubtotal,2) + round(tdcostsubtotal,2)%>
  <tr>
  	<td align="right" colspan=2><b>Sub Total Energy/T&D Charges</b></td>
  	<td width="5">&nbsp;</td>
  	<td align="right"><b><%=FormatCurrency(costsubtotal,2)%></b></td>
  </tr>
  ...
  <%
  dim adminservicetax
  adminservicetax = (cdbl(adminfeecalc) + cdbl(rst2("servicefeenonull"))) * cDbl(.08875)
  
  %>
  <tr>
  	<td align="right" colspan=2><b>Sales Tax(Admin/Service)</b></td>
  	<td width="20">&nbsp;</td>
  	<td align="right"><b><%=FormatCurrency(adminservicetax,2)%></b></td>
  </tr>
  <%
  dim totaldue
  totaldue = costsubtotal + adminservicetax%>
  

  <tr>
  	<td align="right" colspan=2><font size="+1"><b>Total&nbsp;Due</b></font></td>
  	<td width="20">&nbsp;</td>
  	<td align="right"><font size="+1"><b><%=FormatCurrency(totaldue,2)%></b></font></td>
  </tr>
  </table>
  <%masterTotal = totaldue%>
<%
	end select
end select%>
...
<%
  ...
  <% 
	...
	if textheader<>"advRealty.asp" then %><td colspan="3">&nbsp;<br><center><%=txtcontanct%><br>
...
sub miscCredits(numcells, tmpleaseid, billid)
	'response.write "HereCreditnonull" &cdbl(rst2("creditnonull")) &"<BR>"
	'response.write "adjustmentnonull" &rst2("adjustmentnonull") 
	'response.write "MISCCREDITS" & billid
    'response.end
     '  if cint(rst2("creditnonull")) <> 0 or cint(rst2("adjustmentnonull")) <> 0 then
      
      
      if cdbl(rst2("creditnonull")) <> 0 or cint(rst2("adjustmentnonull")) <> 0 then
		...
		if not rstMiscCred.eof then
			do while not rstMiscCred.eof
			    ...
			    creditamount = creditamount + formatcurrency(abs(cdbl(rstMiscCred("credit"))),2)
