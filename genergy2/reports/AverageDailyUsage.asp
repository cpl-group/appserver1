...
dim startdate, enddate, cstartdate, cenddate, currentdate, tmonth, tday, tyear, fmonth, fday, fyear, pdf, selected, groupname, groupid, grouplabel, cmonthst, cyearst, cmonthed, cyeared, weekdays, weekends, holiday, devIP, pid
pdf = request("pdf")
...
if trim(pid)<>"" then pid = " and clientid="&pid else pid=""
...
<tr bgcolor="#6699cc">
  ...
  <%if not(pdf) then%><td align="right"><input type="button" value="Print Current View" onclick="window.open('http://pdfmaker.genergyonline.com/pdfmaker/pdfReport.asp?devIP=<%=request.servervariables("SERVER_NAME")%>&qs=<%=server.URLEncode(request.servervariables("SCRIPT_NAME")&"?"&request.servervariables("QUERY_STRING"))%>','','width=600,height=400,resizable=yes');"></td><%end if%>
</tr>
</table>
...
<tr>
  ...
  <%
  dim i
  if not pdf then response.write "<select name=""groupid"" onchange=""submit();""><option value=""0"">Select a Input Group</option>"
  'response.write "SELECT id, groupname, grouplabel FROM [group] WHERE type=1 and (groupname in (SELECT [name] FROM sysobjects WHERE xtype='v') or groupname in ("&makeIPUnionMulti("sysobjects","[name]","WHERE xtype='v'")&")) "&pid
  'response.end
  rst1.open "SELECT id, groupname, grouplabel FROM [group] WHERE type=1 and (groupname in (SELECT [name] FROM sysobjects WHERE xtype='v') or groupname in ((select [name] from sysobjects WHERE xtype='v'))) "&pid, cnn1
  do until rst1.eof
    selected=""
    if trim(groupid) = trim(rst1("id")) then 
		selected=" SELECTED"
		groupname=rst1("groupname")
    ...
    elseif selected=" SELECTED" then
      response.write rst1("grouplabel")
    end if
    rst1.movenext
  loop
  rst1.close
...
Dates:
  ...
  <select name="cyearst">
    <%
      rst1.open "SELECT distinct datepart(year,datestart) as year FROM billyrperiod b WHERE not(datestart is null) ORDER BY datepart(year,datestart) desc"
      do until rst1.eof
      ...
      rst1.close
    %>
  </select>&nbsp;to&nbsp;<select name="cmonthed">
    <%
      for i = 1 to 12
...
<%if trim(groupid)<>"" and trim(getViewIP(groupid))<>"0" then%>
<%if pdf then makelmpcharts%>
...
rst1.open "SELECT top 1 * FROM ["&groupname&"]", cnnlocal
groupCol = rst1.fields.Item(0).Name
rst1.close
rst1.open "SELECT sum(["&groupCol&"]) as usage, convert(datetime,left(date,11)) as d, (datepart(hour,date)-(datepart(hour,date) % 2))/2 as h FROM ["&groupname&"] WHERE date>='"&cstartdate&"' and date<='"&cenddate&"' GROUP BY convert(datetime,left(date,11)), (datepart(hour,date)-(datepart(hour,date) % 2))/2 ORDER BY d, h", cnnlocal
'response.write "SELECT sum(["&groupCol&"]) as usage, convert(datetime,left(date,11)) as d, (datepart(hour,date)-(datepart(hour,date) % 2))/2 as h FROM ["&groupname&"] WHERE date>='"&cstartdate&"' and date<='"&cenddate&"' GROUP BY convert(datetime,left(date,11)), (datepart(hour,date)-(datepart(hour,date) % 2))/2 ORDER BY d, h"
'response.end
...
%>
  ...
  <%if not(pdf) then%><center><input type="button" value="Load Averages" onclick="submit()"></center>
  <%makelmpcharts%>
<%end if%>
...
<tr>
    ...
    <%if not(pdf) or trim(weekdays)<>"" then%>Selected Weekdays:<%end if%><br>
<%if not(pdf) then%>
...
<%if trim(weekdays)<>"" then%>
    <td <%if not(pdf) then%>align="right"<%end if%>><img src="AverageDailyLMP.asp?startdate=<%=cstartdate%>&enddate=<%=cenddate%>&day=<%=server.urlencode(weekdays)%>&groupname=<%=groupname%>&groupid=<%=groupid%>" border="0" width="525" height="195"></td>
<%end if%>
</tr>
<tr><td align="right" valign="top">
    <%if not(pdf) or trim(weekends)<>"" then%>Selected Weekends:<%end if%><br>
<%if not(pdf) then%>
...
<%if trim(weekends)<>"" then%>
    <td <%if not(pdf) then%>align="right"<%end if%>><img src="AverageDailyLMP.asp?startdate=<%=cstartdate%>&enddate=<%=cenddate%>&day=<%=server.urlencode(weekends)%>&groupname=<%=groupname%>&groupid=<%=groupid%>" border="0" width="525" height="195"></td>
<%end if%>
</tr>
<tr><td align="right" valign="top">
    <%if not(pdf) or trim(holiday)<>"" then%>Selected Holidays:<%end if%><br>
<%if not(pdf) then%>
    <%
