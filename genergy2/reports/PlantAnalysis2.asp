...
<%
dim m, y, building, pid, pdf, groupname, tmonth, tday, tyear, fmonth, fday, fyear, action, patrend, pascatter
...
{ document.location = 'PlantAnalysis.asp?groupname='+id.value
}
...
<tr>
  ...
  <%
  if not pdf then 
    response.write "<select name=""groupname"" ><option value=""0"">Select a Input Group</option>"
    rst1.open "SELECT * FROM [group] WHERE type=5", cnn1
    do until rst1.eof
      selected=""
...
<%if trim(groupname)<>"" and trim(groupname)<>"0" then%>
<tr>
  <td align="center"><img src="http://10.0.7.80/teechart5/PlantAnalysisInOut_trend3.asp?groupname=<%=groupname%>&startdate=<%=tmonth%>/<%=tday%>/<%=tyear%>&enddate=<%=fmonth%>/<%=fday%>/<%=fyear%>&patrend=<%=patrend%>&pascatter=<%=pascatter%>" width="600" height="310"></td>
</tr>
<tr>
  <td align="center"><img src="PlantAnalysisRunHours.asp?groupname=<%=groupname%>&startdate=<%=tmonth%>/<%=tday%>/<%=tyear%>&enddate=<%=fmonth%>/<%=fday%>/<%=fyear%>" width="600" height="350"></td>
</tr>
<tr>
<td align="center">
  <table border="0" cellpadding="3" cellspacing="1" width="600" align="center">
  <tr>
    ...
    <td>
      ...
      do until rst1.eof
        %><tr bgcolor="white"><td><%=rst1("meternum")%></td><td><%=rst1("utilitydisplay")%></td></tr><%
        rst1.movenext
      loop
      rst1.close
      %>
