...
Sub setBuilding(bldgnum, ip,pid,nodename)
On Error Resume Next
  ...
  If Err.number <> 0 then
   response.Write Err.description & "--> pid = " & pid & " bldgnum= " & bldgnum & " ip = " & ip
   response.End()
  End If
...
Function getBuildingIP(bldgnum)
	getBuildingIP = application("superIP")
End Function
...
'    if hasGroup("Genergy Users") then
      rstB.open "SELECT ip as ipout,pid,strt from super_main m inner join portfolio p on p.id = m.pid left join buildings b on b.bldgnum = m.bldgnum WHERE m.bldgnum='"&bldgnum&"'", getConnect(0,0,"dbCore")
      if not rstB.eof then
        if trim(rstB("ipout"))<>"" then
          getBuildingIPCom = rstB("ipout")
          setBuilding bldgnum, getBuildingIPCom,rstB("pid"),rstb("strt")
        end if
      end if
      rstB.close
'    end if
...
Sub setPortfolio(pid, ip,nodename)
	Dim node, dataservers, xml
	Set xml = getXmlSession()
	Set dataservers = xml.selectSingleNode("//dataservers")
...
'    if hasGroup("Genergy Users") then
	...
	rstB.open "SELECT ip as ipout,pid,name from super_main m inner join portfolio p on p.id = m.pid left join buildings b on b.bldgnum = m.bldgnum WHERE m.bldgnum='"&bldgnum&"'", Application("dbdefault") & "dbCore;"

      if not rstB.eof then
        if trim(rstB("ipout"))<>"" then
	      'Proper Handling of NULL
		   getPID = rstB("pid")
           setBuilding bldgnum, rstB("ipout"),getPID, rstB("name")

        end if
      end if
      rstB.close
'    end if
'    if trim(getPID)="" or trim(getPID)="0" then
'      response.write "Error getting portfolio info for '"&bldgnum&"'. Your Session is not configured for this portfolio."
'      response.end
'    end if
  Else 
