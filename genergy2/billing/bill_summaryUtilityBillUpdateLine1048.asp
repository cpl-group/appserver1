...
rst1.close
if allowGroups("Genergy Users") then rpt = "rpt_bill_summary_ROBTESTTAXEXEMPT_915Broadway" else rpt = "rpt_Bill_summary_client_robtest"
...
<td height="68"><img src="http://testserver1.genergy.com/eri_th/pdfMaker/<%=summarylogo%>" hspace="0" width="202" height="143"></td>
...
<%end if%>
	...
	<table border="0" cellspacing="2" cellpadding="5" bgcolor="black" width="250"><tr bgcolor="white"><td><div align="center">Sub-Meter Revenue</div>
		...
		<tr><td><font size="1">Tax Exempt Subtotal</font></td><td><font size="1"><%if isnumeric(bldgSubtotalTaxExempt) then response.write formatcurrency(bldgSubtotalTaxExempt)%></font></td></tr>
		...
		<%if utilityid<>10 and utilityid<>6 and not(rst1.eof) then%>
			...
			<% else %>
			<tr><td><font size="1">% Recoup</font></td><td><font size="1"><%if isnumeric(bldgSubtotal) and trim(rst1("TotalBillAmt"))<>"0" then response.write formatpercent(bldgSubtotal/cdbl(rst1("TotalBillAmt")),2)%></font></td></tr><!-- a bit iffy test this out-->
				<%if utilityid=2 then 
					if ExmpData then %>
   ...
   %>
	<%if ExmpData then%>
		...
		<table border="0" cellspacing="2" cellpadding="5" bgcolor="black" width="250"><tr bgcolor="white"><td><div align="center">Revenue Exempt Summary</div>
			...
			<tr><td colspan="2"><hr size="1" color="#000000" noshade></td></tr>
			<%if utilityid=2 then %>
			    <tr><td><font size="1">% Recoup (KWH)</font></td><td><font size="1"><%if isnumeric(ExmpTotalAmt) then response.write formatpercent(ExmpTotalPeak /cdbl(rst1("TotalKWH")),2) %></font></td></tr>
			<%end if %>    
		</table>
	<%end if%>
	</td></tr></table>
