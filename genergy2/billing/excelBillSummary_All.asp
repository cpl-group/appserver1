...
<html>
<head>
<title>Excel Bill Summary</title>
...
<body>
<table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF" ID="Table1">
  <tr>
   <form name="form1"  action="excelBillSummary_All.asp" ID="Form1">
    <tr bgcolor="#eeeeee"> 
      <td colspan="2" style="border-top:1px solid #ffffff;border-bottom:1px solid #cccccc;"> 
        <table border=0 cellpadding="3" cellspacing="0" ID="Table2">
          <tr> 
			...
			<td> <select name="utilityid" onChange="loadutility()" ID="Select1">
            ...
            <td> <select name="billyear"  onchange="loadperiod()" >
			...
			%>
				...
				<input type="Submit" onclick="loadperiod()" name="Generate Report" value="Generate Report"> 
            </td>
          </tr>
        </table></td>
       </form> 
...
'Set objExcelReport = server.GetObject
Set objWorkBook = objExcelReport.Workbooks.Add
	...
	rst1.open "SELECT umeasure as usage, dmeasure as demand, utilitydisplay as utility " & _
			  "FROM tblutility WHERE UtilityId="&utilityid, getConnect(0,building,"Billing")
...
Do Until rst1.eof
	...
	objWorkSheet.Cells(iRow,12) = rst1("DateStart")
	objWorkSheet.Cells(iRow,13) = rst1("DateEnd")
...
if not rst1.eof then
	...
	objWorkBook.SaveAs("\\10.0.7.21\web_folders\finance\JLL\BillSummary\" & building & Billperiod & Billyear & "BillSummary.xls")
...
strFilename = "C:\ghnet_websites\appserver1\eri_TH\finance\JLL\BillSummary\" & building & Billperiod & Billyear & "BillSummary.xls"
...
'response.Write strFilename
		...
		<a style="font-family:arial;font-size:12;text-decoration:none;color:black;" href="http://appserver1.genergy.com/eri_TH/finance/JLL/BillSummary/<%=building%><%=Billperiod%><%=Billyear%>BillSummary.xls" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'black'"><b><%=building%><%=Billperiod%><%=Billyear%>BillSummary.xls</b></a> 
		</p>
		<%
		Else
		%>
...
{	var frm = document.forms['form1'];
	var newhref = "excelBillSummary_All.asp?bldgNum="+frm.bldgNum.value+"&billyear="+frm.billyear.value;
	document.location.href=newhref;
}

function loadutility()
{	var frm = document.forms['form1'];
	var newhref = "excelBillSummary_All.asp?bldgNum="+frm.bldgNum.value+"&utilityid="+frm.utilityid.value;
	document.location.href=newhref;
}
</Script>
