    ...
    <td width="46%" bgcolor="#0099FF" align="left" style="font-family:arial;font-size:12;text-decoration:none;color:#FFFFFF;" nowrap> 
      ...
      <%else
		%>
			...
			| <a style="font-family:arial;font-size:12;text-decoration:none;color:#FFFFFF;" href="http://pdfmaker.genergyonline.com<%=billlink%>utilityid=<%=utility%>&genergy2=<%=genergy2%>&pid=<%=getpid(trim(building))%>&building=<%=server.urlencode(building)%>&bperiod=<%=bperiod%>&byear=<%=byear%>&demo=<%=demo%>&devIP=<%=request.servervariables("SERVER_NAME")%>&billCount=<%=billCount%>&detailed=true" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'white'"><b>Download&nbsp;all&nbsp;detailed&nbsp;bills&nbsp;for&nbsp;this&nbsp;building</b></a> 
