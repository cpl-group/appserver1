...
<%
dim date1, date2, bldg, utype, pid, ddate, datearray, dateST, dateEN, periodST, periodEN, downloadtype
...
if trim(downloadtype)="submetered" then
	...
	Set prm = cmd.CreateParameter("files", adVarChar, adParamOutput, 50)
	cmd.Parameters.Append prm
	cnn1.getdata bldg, dateST, dateEN, pid
end if
...
<select name="dateST">
	...
	cyear = year(date())
	myear = cyear-5
	rstmin.open "SELECT TOP 1 BillYear FROM BillYrPeriod WHERE BldgNum='" & bldg & "' and billyear >= 2002 ORDER BY BillYear", getLocalConnect(bldg)
...
<td colspan="3" valign="top"><input type="radio" name="downloadtype" value="all" onclick="document.all['ending'].style.display='inline';" checked>&nbsp;Download&nbsp;All<br><input type="radio" name="downloadtype" value="submetered" onclick="document.all['ending'].style.display='none';">&nbsp;Download&nbsp;Submetered&nbsp;</td>
</tr></table>
