...
<tr bgcolor="#3399cc">
	<td><span class="standardheader">
		<%if trim(ypid)<>"" then%>
      Update Bill Period | <span style="font-weight:normal;"><a href="portfolioedit.asp?pid=<%=pid%>" style="color:#ffffff;"><%=portfolioname%></a> &gt; <a href="buildingedit.asp?pid=<%=pid%>&bldg=<%=bldg%>" style="color:#ffffff;"><%=bldgname%></a></span>
		<%else%>
			Add New Bill Period | <span style="font-weight:normal;"><a href="portfolioedit.asp?pid=<%=pid%>" style="color:#ffffff;"><%=portfolioname%></a> &gt; <a href="buildingedit.asp?pid=<%=pid%>&bldg=<%=bldg%>" style="color:#ffffff;"><%=bldgname%></a></span>
		<%end if%>
	</span></td>
</tr>
</table>
...
<tr bgcolor="#eeeeee"> 
	...
	<%if not(isBuildingOff(bldg)) then%>
		...
		<%end if%>
	<%end if%>
		<input type="button" name="cancel" value="Cancel" onclick="location='billPeriodView.asp?pid=<%=pid%>&bldg=<%=bldg%>'" class="standard" style="border:1px outset #ddffdd;background-color:ccf3cc;">
	</td>
</tr>
</table>
