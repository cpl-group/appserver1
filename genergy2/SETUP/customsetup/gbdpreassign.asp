...
function loadmeters()
{
	url = "gbdpreassign.asp?bldg=<%=bldg%>&premise_point=" + document.form1.premise_point.value
...
'	response.end	
	...
	'response.write "exec sp_assigngb '"&premise&"','"&meterPname&"','"&point&"',"&meterid&",'"&datadef&"'"
	'response.end
	cnn.reassign premise, meterPname, point, meterid, datadef
	Set cnn = nothing
	response.redirect "gbdpreassign.asp?bldg="&bldg
elseif request("submit")="SQL String" then
	premisepoint = split(request("premise_point"),"_")
	premise = premisepoint(0)
	meterPname = premisepoint(1)
	point 	= premisepoint(2)
...
<link rel="Stylesheet" href="/genergy2/styles.css" type="text/css">
	...
	<form method="POST" name="form1" action="gbdpreassign.asp">
		...
		<tr valign="top"> 
		  <td>
				...
				<%
					...
					rst1.open "SELECT Distinct meterPname, Premise, point FROM garbage ORDER BY premise, meterPName, point", getConnect(0,bldg,"billing")
					while not rst1.EOF 
      ...
      <input name="bldg" type="hidden" value="<%=bldg%>">
      <input type="submit" name="Submit" value="ASSIGN" onclick="document.all['loadFrame'].style.visibility='visible';formsubmit()">
      <%if allowGroups("IT Services") then%>&nbsp;&nbsp;<input type="submit" name="Submit" value="SQL String"><%end if%>
      </td>
    </tr>
    </table>		  
    </td>
