...
{	document.location = 'billPeriodEditG1.asp?pid=<%=pid%>&bldg=<%=bldg%>&utype=<%=utype%>&ypid='+ypid
}
...
<tr>
  ...
  <tr>
    <td><span class="standardheader">Manage Bill Periods | <span style="font-weight:normal;"><a href="portfolioeditG1.asp?pid=<%=pid%>" style="color:#ffffff;"><%=portfolioname%></a> &gt; <a href="buildingeditG1.asp?pid=<%=pid%>&bldg=<%=bldg%>" style="color:#ffffff;"><%=bldgname%></a></span></span></td>
    <td align="right">
	<label style="border:1px solid #6699cc; color:white; font-weight: bold; border-bottom-style: solid;cursor:hand" onClick="
	document.location='billperiodViewG1.asp?pid=<%=pid%>&bldg=<%=bldg%>&utype=<%=utype%>&historic=<%if historic then%>false<%else%>true<%end if%>'
	...
	<button id="qmark2" onClick="openCustomWin('help.asp?page=billperiodviewG1','Help','width=400,height=500,scrollbars=1')" style="cursor:help;color:#339933;text-decoration:none;height:20px;background-color:#eeeeee;border:1px outset;color:009900;margin-left:4px;" class="standard">(<b>?</b>) Quick Help</button></td>
  </tr>
  </table>
  </td>
</tr>
<tr>
  ...
  <tr>
    ...
    <select name="utility" onChange="document.location='billPeriodViewG1.asp?pid=<%=pid%>&bldg=<%=bldg%>&utype='+this.value+'&historic=<%=historic%>'">
