...
{
	cWin = window.open('/genergy2/billing/processbills.asp?pid=<%=pid%>','Process Bills','width=900,height=1000,resizable=yes, scrollbars=yes')
	cWin.focus();
}
...
<tr bgcolor="#eeeeee">
 ...
 <%  rst5.open "select * from billtemplates where name = 'genergy basic'",cnn1 %>
  <option value="<%=rst5("id")%>"<%if cint(billtemplate)="genergy basic" then response.write " SELECTED"%>><%="genergy basic"%></option>  
      rst5.close
...
<%
  strsql = "SELECT b.bldgname,b.strt,b.city,b.state,b.zip,b.bldgnum FROM buildings b left join regions on b.region=regions.id where portfolioid='" & pid & "' order by offline asc, bldgname asc"
