...
{	document.location.href = 'leaseutilityeditG1.asp?pid=<%=pid%>&bldg=<%=bldg%>&tid=<%=tid%>&lid='+lid
}
...
<form name="form2" method="post" action="tenantsave.asp">
  <table width="100%" border="0" cellpadding="3" cellspacing="0">
      ...
      <td height="26"> <table border=0 cellpadding="0" cellspacing="0" width="100%">
          <tr> 
            <td> <span class="standardheader"> 
              ...
              View
              <% end if %>
              Account | <span style="font-weight:normal;"><nobr><a href="portfolioeditG1.asp?pid=<%=pid%>" style="color:#ffffff;"><%=portfolioname%></a></nobr> 
              &gt; <nobr><a href="buildingeditG1.asp?pid=<%=pid%>&bldg=<%=bldg%>" style="color:#ffffff;"><%=bldgname%></a></nobr> 
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
          ...
          <tr>
			...
			<%if not(isBuildingOff(bldg)) then%>
              ...
              <input type="button" name="action" value="Cancel" onclick="document.location='tenanteditG1.asp?pid=<%=pid%>&bldg=<%=bldg%>&tid=<%=tid%>&edit=0';" class="standard" style="border:1px outset #ddffdd;background-color:ccf3cc;"> 
              &nbsp; 
              <%else%>
    ...
    <tr valign="top" bgcolor="#eeeeee">
     <td>
        <% ' 10/1/2008 by Kamto Cheng - added back the 'Edit Account Information.  requested by the OUC Customer
        if trim(tid)<>"" and not isBuildingOff(bldg) AND (isBuildingTransfered(pid, bldg) = 0) AND pid = 7 then%>
        <input type="button" name="action" value="Edit Account Information" onclick="document.location='tenanteditG1.asp?pid=<%=pid%>&bldg=<%=bldg%>&tid=<%=tid%>&edit=1';" class="standard" style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;margin-top:3px;"> 
        <% end if %>
     </td>
    </tr>
    <%if trim(tid)<>"" then%>
  ...
  <tr>
    ...
    <%
      ...
      if not rst2.EOF then%>
        ...
        <%do until rst2.EOF%>
        <tr bgcolor="#ffffff" onmouseover="this.style.backgroundColor = 'lightgreen'" onmouseout="this.style.backgroundColor = 'white'" onclick="meterEdit1(<%=rst2("meterid")%>,<%=rst1("leaseutilityid")%>);">
        ...
        <%rst2.movenext
        loop%>
        <tr><td colspan="7" style="border-bottom:1px solid #ffffff;"><%if not(isBuildingOff(bldg)) then%><%end if%></td></tr>
      <%
      else
      %>
      ...
      <tr bgcolor="#ffffff">
        <td><span class="standard" style="border-bottom:1px solid #ffffff;"><%if not(isBuildingOff(bldg)) then%><input type="button" value="Add Meter" onclick="meterAdd('',<%=rst1("leaseutilityid")%>);" id=1 name=1 class="standard" style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;"><%end if%></span></td>
      </tr>
      <%
      end if
      rst2.close
