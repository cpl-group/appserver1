...
<table width="100%" border="0" cellpadding="3" cellspacing="0">
  <form>
    ...
    <tr bgcolor="#3399cc"> 
      <td style="border-bottom:1px solid #ffffff;"><span class="standard" style="color:#ffffff">History</td>
    </tr>
  </form>
</table>
...
<tr>
  ...
  <tr>
    ...
    <td>
		...
		<%
			rst1.open "SELECT * FROM ratetypes WHERE regionid='"&rid&"' ORDER BY type", cnn1
				do until rst1.eof
...
<%
if trim(rid)<>""  then
	rst1.Open "SELECT * FROM ratetypes WHERE id='"&rtid&"' ORDER BY type", cnn1
    ...
    '2/15/2008 N.Ambo removed limit to enddate
	  ...
	  sqlstr = "SELECT r.id as rateid, rp.id as rpid, * FROM rate r LEFT JOIN ratepeak rp ON rp.id=r.peak LEFT JOIN rateseasons rs ON rs.id=rp.seasonid LEFT JOIN ratedescription rd ON rd.id=r.linecharge LEFT JOIN tblutility u ON u.utilityid=r.utility WHERE r.type='"&rst1("id")&"' and startdate < '6/1/2009'  ORDER BY itemtype desc, enddate desc, season desc, rateto desc"
