...
dim cnn1, rst1, strsql, ticketcount, masterticketid,opentickets, criticalopentickets, totaltickets
dim pid, edit
...
//  if (parent.frames.length > 2) {
    ...
    parent.contentfrm.location = 'buildingeditG1.asp?pid=<%=pid%>&bldg='+bldg;
//  } else {
<% else %>
    document.location = 'buildingeditG1.asp?pid=<%=pid%>&bldg='+bldg;
<% end if %>
//  }
}
...
<tr bgcolor="#eeeeee">
  ...
  <tr>
    ...
    <%
      rst1.open "SELECT * FROM billtemplates ORDER BY name", cnn1
      do until rst1.eof
  ...
  <tr> 	
    ...
    <td>
      <%if trim(pid)<>"" and (edit) then%>
        ...
        <input type="button" name="cancel" value="Cancel" onclick="location='portfolioeditG1.asp?pid=<%=pid%>&edit=0';" class="standard"  style="border:1px outset #ddffdd;background-color:ccf3cc;">
      <%else%>
...
<%
  ...
  if not rst1.eof then %>	
    ...
    <tr bgcolor="#eeeeee">
      ...
      <td colspan="2" align="right" style="border-top:1px solid #ffffff;border-bottom:1px solid #999999;></td>">
    </tr>
