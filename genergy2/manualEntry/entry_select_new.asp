...
{	var frm = document.forms['form1'];
	var newhref = "entry_select_new.asp?pid="+frm.pid.value+"&scroll="+getScroll();
	document.location.href=newhref;
}

function loadbuilding()
{	var frm = document.forms['form1'];
	var newhref = "entry_select_new.asp?pid="+frm.pid.value+"&building="+frm.building.value+"&scroll="+getScroll();
	document.location.href=newhref;
}

function loadyear()
{	var frm = document.forms['form1'];
	var newhref = "entry_select_new.asp?pid="+frm.pid.value+"&building="+frm.building.value+"&byear="+frm.byear.value+"&scroll="+getScroll();
	document.location.href=newhref;
}
...
{	var frm = document.forms['form1'];
	...
	{	var newhref = "entry_select_new.asp?pid="+frm.pid.value+"&building="+frm.building.value+"&byear="+frm.byear.value+"&bperiod="+bperiod+"&scroll="+getScroll();
		document.location.href=newhref;
	}
}
...
{	var frm = document.forms['form1'];
	var newhref = "entry_select_new.asp?pid="+frm.pid.value+"&building="+frm.building.value+"&byear="+frm.byear.value+"&bperiod=<%=bperiod%>&meterid="+meterid+"&scroll="+getScroll();
	document.location.href=newhref;
}
...
function editReadings(bp,by){
  temp = 'entry_select_new.asp?pid=<%=pid%>&building=<%=building%>&byear='+by+'&bperiod='+bp+'&meterid=<%=meterid%>'+"&scroll="+getScroll();
...
function EditPaneShow(id){
	var tag = document.getElementById(id) 
	tag.style.display = (tag.style.display == "block" ? "none" : "block");
	var func = document.getElementById('func' + id);
...
<form name="form1">
  <table border=0 cellpadding="3" cellspacing="0" width="100%">
    <tr> 
      <td bgcolor="#6699cc"><span class="standardheader">Manual Entry v.2 Test</span></td>
      <td align="right" bgcolor="#6699cc"><% if building <> "" then %><select name="select" onChange="JumpTo(this.value)">
          ...
          <option value="../validation/validation_select.asp">Review Edit</option>
        ...
        <tr valign="bottom"> 
          <td> 
            ...
            <select name="pid" onChange="loadportfolio()">
              ...
              <%
								rst1.open "SELECT distinct p.id, name FROM buildings b, portfolio p WHERE p.id=b.portfolioid ORDER BY p.name", getConnect(pid,building,"dbCore")
								do until rst1.eof%>
...
if trim(building)<>"" then%>
	<table border=0 cellpadding="3" cellspacing="0" width="100%" bgcolor="#ffffff"><%
		if trim(bperiod)<>"0" and trim(bperiod)<>"" then%>
			<tr>
				<td width="12">&nbsp;</td>
				<td class="standard" colspan="4"><b>Unentered meters for period <%=bperiod%> are listed below</b></td>
			</tr>	<%
		else%>
			<tr>
				<td width="12">&nbsp;</td>
				<td class="standard" colspan="4"><b>All online meters are listed below</b>
				</td>
			</tr><%
		end if%>
	<tr><td>&nbsp;</td><td>
	<div id="meterlist" style="overflow:auto; width:97%;height:130px;border:1px solid #cccccc;">
...
<td><font color="#336699"><b><span id="func<%=rst1("billperiod")%>.<%=rst1("billyear")%>" style="cursor:hand" onClick="javascript:EditPaneShow('<%=rst1("billperiod")%>.<%=rst1("billyear")%>')" title="Click Here to Edit This Reading">[+]</span></b></font>
<td><%=rst1("billyear")%></td>
<td><%=rst1("billperiod")%></td>
<%if isnull(rst1("rawcurrent")) then %>
<td colspan="9"><font color="#336699"><b><span style="cursor:help" title="To Add a reading for this period, click the [+] to the left and enter your readings below.">No Readings Entered for this Billperiod</span></b></font></td>
<%else%>
...
<td align="right"><%if lcase(rst1("posted"))="true" then%>Posted<%end if%></td>
<%end if%>
</tr>
<tr style="display:none" id="<%=rst1("billperiod")%>.<%=rst1("billyear")%>"><td colspan=13><% editPane rst1("billyear"),rst1("billperiod") %></td></tr>
<%
'end if
rst1.movenext
loop
...
function printHeaderRow()
  ...
  response.write "  <td width=""1%"" style=""border-top:1px solid #eeeeee;"">Edit</td>"
...
%>
<form name="form_meter_edit" action="entrysave_new.asp" method="post">

<input type="hidden" name="pid" value="<%=pid%>">
<input type="hidden" name="building" value="<%=building%>">
<input type="hidden" name="meterid" value="<%=meterid%>">
<input type="hidden" name="workingperiod" value="<%=billperiod%>">
<input type="hidden" name="workingyear" value="<%=billyear%>">

<table>
<tr><td colspan="11" bgcolor="white">
<table border="0" cellpadding="0" cellspacing="0" width="100%" bgcolor="#cccccc">
<tr bgcolor="#eeeeee">
  <td colspan="2" style="padding:3px;"><b>Period <%=billperiod%> of <%=billyear%></b></td>
  ...
  <%end if%>
  &nbsp;<input name="action" value="Cancel" type="button" onClick="editReadings(0,<%=byear%>);"></td></tr>
<tr valign="top">
<td>
<!-- begin consumption -->
  <table border=0 cellpadding="3" cellspacing="0">
  ...
  <tr>
  	   <td Colspan=7><%=makeInput("usernotec", usernotec)%></td>
  </tr>
  </table>      
 <%
