        ...
        <tr valign="bottom"> 
          <td> 
            ...
            <select name="pid" onChange="loadportfolio()">
              ...
              <%
								dim sqlCommand
					sqlCommand = "(SELECT distinct pt.id, pt.name FROM buildings_dbCore bd, portfolio pt WHERE pt.id=bd.portfolioid)"
								rst1.open sqlCommand & "UNION (SELECT distinct p.id, p.name FROM buildings b, portfolio p WHERE p.id=b.portfolioid) ORDER BY name", getConnect(pid,building,"billing")
								do until rst1.eof%>
