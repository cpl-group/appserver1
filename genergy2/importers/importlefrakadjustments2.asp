...
<%
	...
	mySmartUpload.AllowedFilesList = "jpg,jpeg,png,csv"
	...
	If sFileName <> "" Then
		...
		arrRows = Split(sRows,",")
			...
			if arrRows(0) <> "Building" then 'arrRows(8) <> ""
				strSQL = ""
				strSQL = strSQL &"INSERT INTO Lefrak_ColdWater_BillAdjustments "
				strSQL = strSQL &"(building,Tenant_No_, Tenant_Name, [To], Tenant_Total____, Tenant_Total_Final____, Billid  ) "
				strSQL = strSQL &"VALUES "
				strSQL = strSQL &"('"&arrRows(0)&"','"&arrRows(1)&"','"&arrRows(2)&"' "
				strSQL = strSQL &",'"&arrRows(5)&"','"&arrRows(7)&"','"&arrRows(8)&"','"&arrRows(18)&"') "
...
response.end
		...
		cmd.CommandType = adCmdStoredProc
			...
			cmd.CommandText = "[_Lefrak_Adjustments_Loader]"
		...
		strsql = "select * from Lefrak_AdjustedBills"
