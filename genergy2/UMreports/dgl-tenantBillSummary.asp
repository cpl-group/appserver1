...
dim pid, building, billingid, leaseutilityid, billingaddress, utilitybillname, billyear, pdf, qs, sqft, utilityid
...
<tr valign="bottom"><td></td><td align="right"><%=billingaddress%></td></tr>
...
<td>Available Bill History:</td> 
	<%
		sql = "SELECT distinct year(bbp.datestart) as year FROM billyrperiod byp LEFT JOIN tblbillbyperiod bbp ON bbp.ypid=byp.ypid LEFT JOIN tblmetersbyperiod mbp ON mbp.bill_id=bbp.id WHERE bbp.reject=0 and byp.bldgnum='"&building&"'  and bbp.leaseutilityid="&leaseutilityid&" GROUP BY bbp.DateStart"
		rst1.open sql, cnn1
		do until rst1.eof
	%><td>
		<form name="summaryyear" method="get" action="dgl-tenantBillSummary.asp">
...
<tr bgcolor="#eeeeee"><h2>
    ...
    <td align="right"><b>Total</b></td></h2>
</tr>
...
<!--     <td align="right"><b><%=formatnumber(Totusage,1)%></b></td>
 ...
 -->
    <td align="right"><b><%=formatcurrency(Totsubtotal)%></b></td>
