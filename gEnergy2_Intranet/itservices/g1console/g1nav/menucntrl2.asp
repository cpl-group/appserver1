...
strsql = "select count(*) as revcount, max(revdate) as lastrev  from revtrack where pid in (select portfolio_id from clients where username = '"& Session("loginemail") &"')"
...
<%if not rst1.eof then
	if rst1("revcount") = 1 then 
	
		msg = "Site Revision 1.0" 
	else
		msg = "Revision 1." & rst1("revcount") &", Last Revision Completed " & rst1("lastrev")
	end if 

%>        
        <div align="center"><font face="Arial, Helvetica, sans-serif" size="1" color="#FFFFFF"><a <%if rst1("revcount") <> 0 then %> onclick="openpage('../eri_TH/RevisionTracking/revtrack.asp')" style="cursor:hand" <%end if%> ><%=msg%></a></font></div>
		<%		rst1.close
		Set cnn1=nothing
end if		
%>
