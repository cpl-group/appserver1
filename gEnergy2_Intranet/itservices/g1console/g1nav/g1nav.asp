...
<%
		...
		Do until userdetails.EOF
			...
			if trim(userdetails("customlabel")) = "-1" then
				...
				//bldglabel = userdetails("servicelabel") 5/7/2008 N.Ambo  blocked off this line
				bldglabel	= userdetails("bldgname") //5/7/2008 N.Ambo added this line to replace previous so that branch name will always be the name given to the branch
			end if				
       ...
       %>
	<%=tlevel1%> = theMenu.addEntry(<%=tlevel1%>, "Document", "(c) Genergy 2001", "", "")	
...
if (self.name == 'menu') {
	...
	initialise();
	if (self.location.protocol != "file:") {
		var thePage = pageFromSearch('http://appserver1.genergy.com/g1_clients/mymain.htm', theMenu, true);
	} 
	if (floatingMode) {
