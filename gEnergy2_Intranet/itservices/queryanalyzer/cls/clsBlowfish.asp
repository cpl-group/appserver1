...
Class clsBlowfish
	...
	Public Function Enc(strData)
		Enc = blf_String(blf_StringPad(strData), true)
	End Function

	Public Function Dec(strData)
		Dec = blf_StringUnPad(blf_String(strData, false))
	End Function

	Public Function SetKey(ByVal szTxtKey)
		Dim nKeyLen
		SetKey = False
		nKeyLen = len(szTxtKey)
		If nKeyLen = 0 Or nKeyLen > 56 Then Exit Function
		Call blf_KeyInit(szTxtKey)
		SetKey = True
	End Function
	...
	Private Function blf_String(strData, bEncrypt)
		Dim strIn, strOut, nLen, nBlocks, sBlock
		Dim i, j, aBytes(7), iIndex,x
		strIn = strData
		nLen = Len(strIn)
		nBlocks = nLen \ 8
		strOut = ""
	...
	Private Function blf_StringPad(strData)
		Dim nLen, nPad, sPad
		nLen = Len(strData)
		nPad = ((nLen \ 8) + 1) * 8 - nLen
		sPad = String(nPad, Chr(nPad))  ' Pad with # of pads (1-8)
		blf_StringPad = strData & sPad
	End Function

	Private Function blf_StringUnPad(strData)
		Dim nPad, nLen
		nLen = Len(strData)
		nPad = Asc(Right(strData, 1))
		If nPad > 8 Then nPad = 0
		blf_StringUnPad = Left(strData, nLen - nPad)
	End Function

	Private Function blf_KeyInit(szTxtKey)
		Dim i, j, K
		Dim nKeyLen, wData, wDataL, wDataR, akey()
  		ReDim aKey(Len(szTxtKey) - 1)
		...
		For i = 0 To (16 + 1)
			...
			For K = 0 To 3
				...
				j = j + 1
				If j >= nKeyLen Then j = 0
			Next
			blf_P(i) = blf_P(i) Xor wData
		Next
		wDataL = &H0
		wDataR = &H0
		For i = 0 To (16 + 2 - 1) Step 2
	...
	Private Function blf_EncipherBlock(xL, xR)
		Dim i
		Dim temp
		For i = 0 To 16 - 1
			xL = xL Xor blf_P(i)
		...
		xR = temp
		xR = xR Xor blf_P(16)
		xL = xL Xor blf_P(16 + 1)
	End Function

	Private Function blf_DecipherBlock(xL, xR)
		Dim i
		Dim temp
		For i = 16 + 1 To 2 Step -1
			xL = xL Xor blf_P(i)
			xR = blf_F(xL) Xor xR
			temp = xL
			xL = xR
	...
	Private Function blf_EncryptBytes(aBytes())
		Dim wordL, wordR
	...
	Private Function blf_DecryptBytes(aBytes())
		Dim wordL, wordR
		wordL = uwJoin(aBytes(0), aBytes(1), aBytes(2), aBytes(3))
		wordR = uwJoin(aBytes(4), aBytes(5), aBytes(6), aBytes(7))
	...
	Private Function bu_HexStr2Bytes(str, aBytes())
		nBytes = Len(str) \ 2
		For i = 0 To nBytes - 1
	...
	Private Function bu_Bytes2HexStr(aBytes(), nBytes)
		Dim strTemp
	...
	Private Function bu_String2Bytes(str, aBytes())
		nBytes = Len(str)
		For i = 0 To nBytes - 1
	...
	Private Function bu_Bytes2String(aBytes(), nBytes)
		For i = 0 To nBytes - 1
	...
	Private Function bu_Str2Hex(str)
		sHex = ""
		n = Len(str)
		For i = 1 To n
	...
	Private Function uwJoin(a, b, C, d)
	...
	Private Sub uwSplit(ByVal w, a, b, C, d)
	...
	Private Function uw_ShiftLeftBy8(wordX)
	...
	Private Function uw_WordAdd(wordA, wordB)
		Dim myUnsigned
		myUnsigned = LongToUnsigned(wordA) + LongToUnsigned(wordB)
		If myUnsigned > 4294967296 Then
			myUnsigned = myUnsigned - 4294967296
		End If
		uw_WordAdd = UnsignedToLong(myUnsigned)

	End Function

	Private Function uw_WordSub(wordA, wordB)
		Dim myUnsigned
		myUnsigned = LongToUnsigned(wordA) - LongToUnsigned(wordB)
	...
	Private Function blf_LoadArrays()
		...
		&H9216D5D9, &H8979FB1B)
		For i = 0 To 17: blf_P(i) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(0, i) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(0, i + 64) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(0, i + 128) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(0, i + 192) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(1, i) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(1, i + 64) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(1, i + 128) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(1, i + 192) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(2, i) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(2, i + 64) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(2, i + 128) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(2, i + 192) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(3, i) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(3, i + 64) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(3, i + 128) = vntA(i): Next
		...
		For i = 0 To 63: blf_S(3, i + 192) = vntA(i): Next
