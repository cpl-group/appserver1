 ...
 <% if trim(company)<>"AC" and trim(company)<>"" then %>
  ...
  <%
    if pid="" then 'portfolios and ungrouped customers, switch js params accordingly
		...
		dim mainIp, someSQL
		mainIp = getIpFromCnnStr(getConnect(0,0,"intranet"))
		someSQL = "select portfolio,name,'0' as customer from dbo.portfolio union select 'xxx' as portfolio,name,customer from main.dbo." & company & "_master_arm_customer where PID='PID' order by portfolio,name "
		'response.write someSQL
		'response.end
		rst.Open  someSQL, getConnect(0,0,"intranet")
		do until rst.eof
      %>
