...
function initOutlineIcons(imgStore) {
	...
	if (self.location.protocol == "file:") {
		imgStore.add('OfflineRoot', 'images/menu/offlineroot.gif', 18, 16);
	}
}
...
%>
	...
	if (self.location.protocol != "file:") {
		level1ID = theMenu.addEntry(level1ID, "Document", "<%Response.Write(userinfo("company"))%>", "mymain.htm", "Welcome to Genergy.com");
    ...
    %>
	level1ID = theMenu.addEntry(level1ID, "Folder", "Presentation", "presentation/present_1_5.htm", "main");
	if (self.location.protocol == "file:") {
		level1ID = theMenu.addEntry(level1ID, "Folder", "", "javascript:void(0)", "");
		theMenu.entry[level1ID].isopen = true;
		theMenu.entry[level1ID].noOutlineImg = true;
		theMenu.entry[level1ID].type = "OfflineRoot";
	} else {
		level1ID = theMenu.addEntry(level1ID, "Folder", "<%Response.Write(userdetails("region"))%>", "", "");
        ...
        %>
	level2ID = theMenu.addChild(level1ID, "Folder", "<%Response.Write(userdetails("bldg_name"))%>", "", "");
	
	theMenu.entry[level2ID].multiLine = true;
			<%      if userdetails("pgi") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "One-line Power Grid Diagram (PGI)", "<%Response.Write(userdetails("pgi_link"))%>", "");
            <%
                    end if
	                if userdetails("pow_ava") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Power Availability Chart", "<%Response.Write(userdetails("pow_ava_link"))%>", "");
            <%
                    end if
                    if userdetails("pa_chart") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Power Availability Details", "<%Response.Write(userdetails("pa_chart_link"))%>", "");
            <%
                    end if
                    if userdetails("lmp") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Load Management Profile (LMP)", "<%Response.Write(userdetails("lmp_link"))%>",  "");
            <%
                    end if
	                if userdetails("meter") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Tenants Sub-Metering", "<%Response.Write(userdetails("meter_link"))%>", "");
            <%
                    end if
                    if userdetails("eri") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Tenants Electric Rent Inclusion (ERI)", "<%Response.Write(userdetails("eri_link"))%>", "");
            <%
                    end if
                    if userdetails("plp") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Public Light & Power Calculations (PLP)", "<%Response.Write(userdetails("plp_link"))%>", "");
            <%
                    end if
                    if userdetails("ovthvac") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Overtime HVAC Calculations", "<%Response.Write(userdetails("ovthvac_link"))%>",  "");
            <%
                    end if
                    if userdetails("iri") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Infrared Maintenance (IRI)", "<%Response.Write(userdetails("iri_link"))%>", "");
            <%
                    end if
					if userdetails("msi") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "MSI", "<%Response.Write(userdetails("msi_link"))%>", "");
            <%
                    end if
					if userdetails("mep") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "MEP", "<%Response.Write(userdetails("mep_link"))%>", "");
            <%
                    end if
	                if userdetails("pqm") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Power Quality Monitoring", "<%Response.Write(userdetails("pqm_link"))%>", "");
            <%
                    end if
	                if userdetails("cs_rtm") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Critical Systems Real-Time Monitoring", "<%Response.Write(userdetails("cs_rtm_link"))%>", "");
            <%
                    end if
                    if userdetails("rev_prof") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Building Revenue Profile", "<%Response.Write(userdetails("rev_prof_link"))%>", "");
            <%
                    end if
                    if userdetails("ca") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Building Cost Analysis", "<%Response.Write(userdetails("ca_link"))%>", "");
            <%
                    end if
...
userinfo.close
        %>
	level2ID = theMenu.addChild(level1ID, "Folder", "Basic Formulas", "", "")
	level3ID = theMenu.addChild(level2ID, "Document", "True and Apparent Power", "presentation/present_6.htm", "");
	level3ID = theMenu.addChild(level2ID, "Document", "Electric Current (Amperes)", "presentation/present_8.htm", "");
	level3ID = theMenu.addChild(level2ID, "Document", "Voltage Drop", "presentation/present_ex_vd.htm", "");
	level3ID = theMenu.addChild(level2ID, "Document", "Watts per Square Foot", "presentation/present_7.htm", "");
	level3ID = theMenu.addChild(level2ID, "Document", "Ampacity of Conductors - NEC", "presentation/present_ex_acnec.htm", "");
	level3ID = theMenu.addChild(level2ID, "Document", "Ampacity of Conductors - NYEC", "presentation/present_ex_acnycec.htm", "");
	level3ID = theMenu.addChild(level2ID, "Document", "Dimensions of RCTCC", "presentation/present_ex_dimensions.htm", "");
	level3ID = theMenu.addChild(level2ID, "Document", "Dimensions of Conduit Tubing", "presentation/present_ex_dimensions_tube.htm", "");

	level3ID = theMenu.addChild(level2ID, "Document", "Example 1", "presentation/present_ex1.htm", "");
	level3ID = theMenu.addChild(level2ID, "Document", "Example 2", "presentation/present_ex2.htm", "");
	level3ID = theMenu.addChild(level2ID, "Document", "Example 3", "presentation/present_ex3.htm", "");

	level1ID = theMenu.addEntry(level1ID, "Folder", "Site Utilities", "", "")
	level2ID = theMenu.addChild(level1ID, "Document", "On-Line Manual", "manual/contents.htm", "");
	level2ID = theMenu.addChild(level1ID, "Document", "Plug-in Download", "downloads.htm", "");
	level2ID = theMenu.addChild(level1ID, "Document", "Other Services offered by Genergy", "otherservices.htm", "");
	level2ID = theMenu.addChild(level1ID, "Document", "Help", "help.htm", "");
	level1ID = theMenu.addEntry(level1ID, "Document", "(c) Genergy 2001", "", "")	
}
...
if (self.name == 'menu') {
	...
	initialise();
	if (self.location.protocol != "file:") {
		var thePage = pageFromSearch('https://appserver1.genergy.com/g1_clients/mymain.htm', theMenu, true);
	} 
	if (floatingMode) {
		self.document.writeln('<frameset cols="100%" rows="*,48" BORDER="0" FRAMEBORDER="0" FRAMESPACING="0" onResize="defaultResizeHandler();">');
		self.document.writeln('<frame name="menu" src="menu2.htm" scrolling="auto" marginwidth="1" marginheight="1">');
		self.document.writeln('<frame name="menuCntrl" src="menucntrl2.htm" scrolling="no" marginwidth="0" marginheight="0">');
		self.document.writeln('</frameset>');
	} else {
		self.document.writeln('<frameset  cols="100%" rows="30,*" BORDER="0" FRAMEBORDER="0" FRAMESPACING="0" onResize="defaultResizeHandler()"; >');
		self.document.writeln('<frame name="title" src="banner.htm" FRAMEBORDER="0" scrolling="no" noresize marginwidth="0" marginheight="0">');
		self.document.writeln('<frameset cols="271,*" rows="100%">');
		self.document.writeln('<frameset cols="100%" rows="*,180">');
		self.document.writeln('<frame name="menu" src="menu2.htm" scrolling="auto" marginwidth="1" marginheight="1">');
		self.document.writeln('<frame name="menuCntrl" src="menucntrl2.asp"  scrolling="no" marginwidth="0" marginheight="0">');
		self.document.writeln('</frameset>');
