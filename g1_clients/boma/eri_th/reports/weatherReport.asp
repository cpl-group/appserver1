...
{ document.location = 'weatherReport.asp?pid=<%=pid%>&building='+building+'&utilityid=2'
}
...
<tr>
  ...
  rst1.open "SELECT * FROM buildings WHERE portfolioid='"&pid&"'", cnn1
  do until rst1.eof
    selected=""
    if trim(building) = trim(rst1("bldgnum")) then selected=" SELECTED"
    if not(pdf) then
      response.write "<option value="""&rst1("bldgnum")&""""&selected&">"&rst1("strt")&"</option>"
    elseif selected=" SELECTED" then
      response.write rst1("strt")
    end if
  ...
  %>
  <%
  dim bperiod, byear, custom_dates
  custom_dates = true
  if not pdf then
  rst1.open "SELECT * FROM billyrperiod WHERE bldgnum='"&building&"' AND datestart<getdate() ORDER BY datestart desc", cnn1
...
'response.write "exec sp_LMPDATA '"&startdate&"','"&enddate&"','b','"&building&"',"&utilityid&",0,0"
'response.end
...
%>
<tr>
  <td align="center"><img src="usageBarChart.asp?building=<%=building%>&startdate=<%=startdate%>&enddate=<%=enddate%>&utilityid=<%=utilityid%>&dutcdate=<%=dutcdate%>&uddusage=<%=uddusage%>&udddegree=<%=udddegree%>" width="650" height="320"></td>
</tr>
<tr>
  <td align="center">
  <%if not pdf then%>
...
cmd.Parameters("utility") = utilityid
'response.write "cmd.execute '"&building&"', '"&startdate&"', '"&enddate&"', "&utilityid
