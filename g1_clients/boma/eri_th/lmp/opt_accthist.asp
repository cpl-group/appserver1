...
{   <%if luid<>"" then%>
        var temp = '../meter/meterbillsummary2.asp?Y='+ ypid +'&l=<%=luid%>&m=<%=meterid%>&b='+bldg;
    <%else%>
...
if luid<>"" then
    ...
    sql = "select u.OnPeakKWH,u.OffPeakKWH,u.TotalKW, u.TotalKWH, u.TotalBillAmt,datestart, dateend,u.ypid from utilitybill u join billyrperiod on billyrperiod.ypid = u.ypid where u.ypid in (SELECT top 12 tblbillbyperiod.ypid FROM tblbillbyperiod where bldgnum='" & bldg & "'  group by tblbillbyperiod.ypid order by tblbillbyperiod.ypid desc ) order by billyear desc, billperiod desc"
end if
