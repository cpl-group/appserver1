...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
  <tr>
    <td width="51%" valign="top"><font face="Arial, Helvetica, sans-serif" size="2"><b> 
      <%
'addons available
rst1.open "SELECT Label, Link, Target, Active FROM tbladdonlinks JOIN tbladdons on tbladdons.SID=tbladdonlinks.SID WHERE CSID=4 and userid='"&session("userid")&"' ORDER BY listorder", cnn1
...
rst1.open "SELECT Label FROM tbladdons WHERE SID not in (SELECT SID FROM tbladdonlinks WHERE userid='" &session("userid")& "' and active=1) AND CSID=4 ORDER BY listorder", cnn1
do while not(rst1.eof)
