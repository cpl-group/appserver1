...
'        response.write "<nobr><a href=""lmpload2.asp?b="& b &"&luid="& luid &"&m="& m &""">"
        ...
        do until rst1.EOF
            ...
            response.write ">"& rst1("tenantnum") &" - Demo Tenant</option>"
            rst1.movenext
        loop
        rst1.close
...
%>
      ...
      <td width="21%" valign="top"> 
        <div name="meterlist" style="border-width:1; width:120; height:200;overflow:auto" > 
          <table border="0" cellpadding="0" cellspacing="0" width="100%">
            ...
            <%if leaseid <> "" then %>
            <tr> 
              <td bgcolor="#0099FF" valign="top"> 
                <div align="center"><font color="#000000" face="Arial, Helvetica, sans-serif" size="1"><b><font face="Arial, Helvetica, sans-serif" size="3"> 
            ...
            <%
				if leaseid <> "" then
    				...
    				strsql = "SELECT DISTINCT meters.meterid, p.meterid as hasdata, meternum, lmnum from meters LEFT OUTER JOIN pulse_"& b &" p on meters.meterid=p.meterid where (LeaseUtilityId=" & leaseid & "and online=1 and lmnum is not NULL) or (leaseUtilityId=" & leaseid & "and online=1 and EXISTS (select * from tblLeasesUtilityPrices where LeaseUtilityId=" & leaseid &  " and LoadProfile=1))order by meternum"
            ...
            <tr> 
              <td onMouseOver="this.style.background='lightgreen'" onMouseOut="this.style.background='white'" height="19"><a <%if IsNumeric(trim(rst1("hasdata"))) then%>onClick="loadmeter(<%=rst1("meterid")%>)" href="lmpload2.asp?m=<%=rst1("meterid")%>&b=<%=b%>&tenantmeter=1&luid=<%=luid%>"<%end if%> target="lmp" style="text-decoration:none; color:black; font-size:12px; font-family:arial"><%= italic &rst1("meternum")& italicx %></a></td>
            </tr>
            <%rst1.movenext
		    		loop
				End if%>
    ...
    <tr>
      <td height="20" width="789" valign="bottom" bgcolor="#000000"> 
        <div align="left"><a href="javascript:meterfill(document.forms['lmp'].leaseid.value, document.forms['lmp'].b.value, document.forms['lmp'].profiletype.value)" style="text-decoration:none;" onMouseOver="this.style.color = 'lightblue'" onMouseOut="this.style.color = 'white'"><font face="Arial, Helvetica, sans-serif" size="3"><b>View Selected Aggregated Profile </b></font></a></div>
      </td>
