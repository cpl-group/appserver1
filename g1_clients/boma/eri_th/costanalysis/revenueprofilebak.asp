...
for x = 1 to numRecords
	if rst1("Expenses") <> "" then i(x, 1) = Formatnumber(rst1("Expenses")) else i(x,1)=0 end if
	if rst1("Submetered") <> "" then i(x, 2) = Formatnumber(rst1("Submetered")) else i(x,2)=0 end if
	if rst1("eri_rev") <> "" then i(x, 3) = Formatnumber(rst1("eri_rev")) else i(x,3)=0 end if
	i(x, 4) = ccur(i(x,2)) + ccur(i(x,3)) - ccur(i(x,1))
	tots = rst1("Expenses")	
rst1.MoveNext
tot=tot+tots
next
...
<SCRIPT LANGUAGE="javascript">
		...
		BarGraphOBJ.style.posLeft = (<%if r=1 then Response.Write PosLeft-5 else response.write PosLeft+3 end if%>);
		<% 
