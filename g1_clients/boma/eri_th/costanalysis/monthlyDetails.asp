...
<tr><td valign="top" width="106">
	...
	<%if ArrPrefs(exps) then%><tr style="font-family: Arial, Helvetica, sans-serif; font-size: 10;"><td><a href="BreakdownExpense.asp?b=<%=b%>&pid=<%=pid%>&date1=<%=date1%>&date2=<%=date2%>&utype=<%=utype%>" style="color:black">Expenses</a></td></tr><%end if%>
	<%if ArrPrefs(subm) then%><tr style="font-family: Arial, Helvetica, sans-serif; font-size: 10;"><td><u><a style="cursor:hand" onclick="window.open('BreakdownSub_eri.asp?b=<%=b%>&pid=<%=pid%>&date1=<%=date1%>&date2=<%=date2%>&utype=<%=utype%>&adjtype=sub','Submeter','width=800,height=600,toolbar=no,scrollbars=yes,resizable=yes')" style="color:black">Submeter</a></u></td></tr><%end if%>
	<%if ArrPrefs(eri) then%><tr style="font-family: Arial, Helvetica, sans-serif; font-size: 10;"><td><a href="BreakdownSub_eri.asp?b=<%=b%>&pid=<%=pid%>&date1=<%=date1%>&date2=<%=date2%>&utype=<%=utype%>&adjtype=eri" style="color:black">ERI</a></td></tr><%end if%>
	<%if ArrPrefs(urar) then%><tr style="font-family: Arial, Helvetica, sans-serif; font-size: 10;"><td><a href="BreakdownAdjustment.asp?b=<%=b%>&pid=<%=pid%>&date1=<%=date1%>&date2=<%=date2%>&utype=<%=utype%>&adjtype=rev" style="color:black">Revenue&nbsp;Adjustment</a></td></tr><%end if%>
	<%if ArrPrefs(urae) then%><tr style="font-family: Arial, Helvetica, sans-serif; font-size: 10;"><td><a href="BreakdownAdjustment.asp?b=<%=b%>&pid=<%=pid%>&date1=<%=date1%>&date2=<%=date2%>&utype=<%=utype%>&adjtype=exp" style="color:black">Expense&nbsp;Adjustment</a></td></tr><%end if%>
	<%if ArrPrefs(mac) then%><tr style="font-family: Arial, Helvetica, sans-serif; font-size: 10;"><td><a href="BreakdownPLP_mac.asp?b=<%=b%>&pid=<%=pid%>&date1=<%=date1%>&date2=<%=date2%>&utype=<%=utype%>&adjtype=mac" style="color:black">Mac&nbsp;Adjustment</a></td></tr><%end if%>
	<%if ArrPrefs(plp) then%><tr style="font-family: Arial, Helvetica, sans-serif; font-size: 10;"><td><a href="BreakdownPLP_mac.asp?b=<%=b%>&pid=<%=pid%>&date1=<%=date1%>&date2=<%=date2%>&utype=<%=utype%>&adjtype=plp" style="color:black">PLP</a></td></tr><%end if%>
...
<a href="/eri_th/pdfMaker/pdfMonthlyDetails.asp?<%=request.servervariables("QUERY_STRING")&"&preferences="&join(arrprefs,",")&","&pid%>" target="_new" style="font-family: Arial, Helvetica, sans-serif; font-size: 11;color:black;">View Printable Report</a></span>
</body>
</html>
