...
cnn1.Open "driver={SQL Server};server=10.0.7.110;uid=genergy1;pwd=g1appg1;database=security;"
...
%>
    ...
    level3ID = theMenu.addChild(level2ID, "Document", "Tenants Electric Rent Inclusion (ERI)", "<%Response.Write(userdetails("eri_link"))%>", "");
            ...
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "On-line Power Grid Diagram (PGI)", "<%Response.Write(userdetails("pgi_link"))%>", "");
            <%
                    end if
...
userinfo.close
        %>
	level1ID = theMenu.addEntry(level1ID, "Folder", "Utilities", "", "")
...
if (self.name == 'menu') {
	...
	if (floatingMode) {
		self.document.writeln('<frameset cols="100%" rows="*,48" BORDER="0" FRAMEBORDER="0" FRAMESPACING="0" onUnload="unloadFloating();" onResize="defaultResizeHandler();">');
		...
		self.document.writeln('<frameset onload="maxm()" cols="100%" rows="10,*" BORDER="0" FRAMEBORDER="0" FRAMESPACING="0" onResize="defaultResizeHandler()"; onunload="logoff()">');
		self.document.writeln('<frame name="title" src="null.htm" FRAMEBORDER="0" scrolling="no" noresize marginwidth="0" marginheight="0">');
