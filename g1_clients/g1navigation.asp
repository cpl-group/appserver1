...
strsql = "SELECT * FROM clients WHERE (username='" & trim(Session("loginemail")) & "') "  
userinfo.Open strsql, cnn1, adOpenStatic

strsql = "SELECT * FROM clientsites WHERE (userid='" & trim(Session("loginemail")) & "') order by regioncount,bldgid"  
...
%>
	...
	//level1ID = theMenu.addEntry(level1ID, "Folder", "The Joust Outliner", "", "");
    <% 
        r=0
	...
	theMenu.entry[level2ID].multiLine = true;
            ...
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Load Management", "<%Response.Write(userdetails("lmp_link"))%>",  "");
            <%
                    end if
                    if userdetails("eri") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "ERI Management Profile", "<%Response.Write(userdetails("eri_link"))%>", "");
            <%
                    end if
                    if userdetails("pgi") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Power Grid Identification", "about:blank", "");
	theMenu.entry[level3ID].onClick = "openwin('<%=userdetails("pgi_link")%>',725,650);";
            <%
                    end if
    ...
    level3ID = theMenu.addChild(level2ID, "Document", "MEP", "<%Response.Write(userdetails("mep_link"))%>", "");
            ...
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Power Availability", "<%Response.Write(userdetails("pow_ava_link"))%>", "");
            <%
                    end if
                    if userdetails("pa_chart") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Power Chart", "<%Response.Write(userdetails("pa_chart_link"))%>", "");
            <%
                    end if
                    if userdetails("rev_prof") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Revenue Profile", "<%Response.Write(userdetails("rev_prof_link"))%>", "");
            <%
                    end if
                    if userdetails("ovthvac") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Over Time HVAC", "<%Response.Write(userdetails("ovthvac_link"))%>",  "");
            <%
                    end if
                    if userdetails("plp") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "PLP Profile", "<%Response.Write(userdetails("plp_link"))%>", "");
            <%
                    end if
                    if userdetails("iri") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "IRI", "<%Response.Write(userdetails("iri_link"))%>", "");
            <%
                    end if
                    if userdetails("meter") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Meter Services", "<%Response.Write(userdetails("meter_link"))%>", "");
            <%
                    end if
                    if userdetails("ca") then 
            %>             
    level3ID = theMenu.addChild(level2ID, "Document", "Cost Analysis", "<%Response.Write(userdetails("ca_link"))%>", "");
            <%
                    end if
                 end if
