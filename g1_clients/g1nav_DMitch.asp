...
strsql = "SELECT clientsetup.id as lid,  * FROM clientsetup inner join services on services.id=serviceid WHERE (userid='"&userid&"') ORDER BY catid, regioncount, bldgorder,serviceid"
...
%>
	if (self.location.protocol != "file:") {
		<%=level1%> = theMenu.addEntry(<%=level1%>, "Document", "<%Response.Write(userinfo("company"))%>", "DMitch.htm", "Welcome to Genergy.com");
	}
<%
		...
		Do until userdetails.EOF
			...
			if trim(userdetails("customlabel")) = "-1" then
				...
				bldglabel = userdetails("servicelabel")
			end if				
...
if (self.name == 'menu') {
	...
	initialise();
	if (self.location.protocol != "file:") {
		var thePage = pageFromSearch('http://testserver1.genergy.com/g1_clients/DMitch.htm', theMenu, true);
	} 
	if (floatingMode) {
