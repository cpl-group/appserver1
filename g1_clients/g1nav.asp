...
'6/13/2008 N.Ambo modified line to assign correct value to bldg label
...
strsql = "SELECT clientsetup.id as lid,  * FROM clientsetup inner join services on services.id=serviceid WHERE (userid='"&userid&"') ORDER BY catid, regioncount, bldgorder, ViewOrderSeq"
...
%>
	...
	tlevel5 =  "level5ID"
		...
		Dim currentBLDG, currentREGION
		regcnt=userinfo("regioncount") 		
%>
	if (self.location.protocol != "file:") {
		<%=level1%> = theMenu.addEntry(<%=level1%>, "Document", "<%Response.Write(userinfo("company"))%>", "mymain.htm", "Welcome to Genergy.com");
	}
<%

		Do until userdetails.EOF
			currentBLDG 		= userdetails("bldgid")
			currentREGION 		= userdetails("regioncount")				
			currentCat 			= userdetails("catid")
			currentServiceID	= userdetails("serviceid")

			if trim(userdetails("customlabel")) = "-1" then
				servicelabel = userdetails("servicelabel")
				bldglabel	= userdetails("bldgname")
			else
				servicelabel = userdetails("customlabel")
				//bldglabel = userdetails("servicelabel") '6/13/2008 N.Ambo blocked off and replaced so that bldglable always reads the bldg name
				bldglabel = userdetails("bldgname")
			end if				

			if trim(currentCat) = "-1" then 
				if trim(userdetails("region")) = "-1" then 
					if trim(currentBLDG) = "-1" then
							level1 =  "level1ID"
							level2 =  "level1ID"
							level3 =  "level1ID"
							level4 =  "level2ID"
			 ...
			 if strcomp(trim(currentCat), trim(lastCat)) = 0 then 
				if cint(currentREGION) = cint(lastregion) then 
					if (strcomp(trim(currentBLDG), trim(lastbldg)) = 0 and (trim(userdetails("customlabel")) <> trim(userdetails("bldgname")))) or (strcomp(trim(currentBLDG), trim(lastbldg)) = 1 and (trim(currentServiceID)) = trim(lastServiceid) and (trim(userdetails("customlabel")) = trim(userdetails("bldgname")))) then 
						currentLevel = "newService"
					else
						currentLevel = "newBldg"
					end if
				else
					currentLevel = "newRegion"
				end if
			else
				currentLevel = "newCat"
			end if
			
			if trim(request("mode")) = "edit" then 
				if (trim(userdetails("customlabel")) = trim(userdetails("bldgname"))) then				
					bldglink = "../leaf.asp?lid=" & userdetails("lid")
					servicelink 	= "../branch.asp?lid=" & userdetails("lid")
				else
					servicelink = "../leaf.asp?lid=" & userdetails("lid")
					bldglink 	= "../branch.asp?lid=" & userdetails("lid")
				end if
				regionlink	= "../region.asp?lid=" & userdetails("lid")
				catlink	= "../category.asp?lid=" & userdetails("lid")						  
			else
				if trim(userdetails("servicelink")) <> "NA" then 
					if (trim(userdetails("customlabel")) = trim(userdetails("bldgname"))) then
						bldglink = userdetails("servicelink") & userdetails("serviceurl")				
						servicelink = ""
					else
						bldglink = ""
						servicelink = userdetails("servicelink") & userdetails("serviceurl")				
					end if
				else
					if (trim(userdetails("customlabel")) = trim(userdetails("bldgname"))) then
						bldglink = userdetails("serviceurl")				
						servicelink = ""
					else
						bldglink = ""
						servicelink = userdetails("serviceurl")				
					end if
				end if
			end if

			Select Case trim(currentLevel)
			    Case "newCat"
					 if trim(currentCat) = "-1" then 
						if trim(userdetails("region")) <> "-1" then %> 
						<%=level1%> = theMenu.addEntry(<%=level1%>, "Folder", "<%Response.Write(userdetails("region"))%>", "<%=regionlink%>", "");
					   <%
					   		entryType = "addChild"
					   	else
							entryType = "addEntry"
					...
					%> 
					<%=level0%> = theMenu.addEntry(<%=level0%>, "Folder", "<%Response.Write(userdetails("category"))%>", "<%=catlink%>", "");
					<%=level1%> = theMenu.addChild(<%=level0%>, "Folder", "<%Response.Write(userdetails("region"))%>", "<%=regionlink%>", "");
				<% 
					entryType = "addChild"
				end if %>
				
				<% if trim(currentBLDG) <> "-1" then %>
						<%=level2%> = theMenu.<%=entryType%>(<%=level1%>, "Folder", "<%Response.Write(bldglabel)%>", "<%=bldglink%>", "");			
						theMenu.entry[<%=level2%>].multiLine = true;
						
				<% 		entryType = "addChild"
				end if 
					if servicelink <> "" then 	%>																									
					<%=level3%> = theMenu.<%=entryType%>(<%=level2%>, "Document", "<%Response.Write(servicelabel)%>", "<%Response.Write(servicelink)%>",  "");
					<%					
					end if
						
					if trim(userdetails("servicetarget")) <> "main" and servicelink <> "" then
					%>
						theMenu.entry[<%=level3%>].target = "openwin('<%=userdetails("servicetarget")%>',725,650);";
					<%
					end if 

				lastregion = currentREGION
				lastbldg = currentBLDG
				lastcat = currentCat
				...
				Case "newRegion"
					%>
						if (self.location.protocol == "file:") {
							<%=level1%> = theMenu.addEntry(<%=level1%>, "Folder", "", "javascript:void(0)", "");
							theMenu.entry[<%=level1%>].isopen = true;
							theMenu.entry[<%=level1%>].noOutlineImg = true;
							theMenu.entry[<%=level1%>].type = "OfflineRoot";
						} else 
							
							<% if trim(userdetails("region")) = "-1" then 
									entrytype = "addEntry"
							else 
									if cint(lastREGION) <> cint(currentREGION) then 
											entrytype = "addEntry"
									else
											entryType = "addChild"
									end if %>									
							
							{      //alert('<%=entryType & lastregion & currentregion%>');
									<%=level1%> = theMenu.<%=entryType%>(<%=level1%>, "Folder", "<%Response.Write(userdetails("region"))%>", "<%=regionlink%>", "");							
						}
							<% 
											entryType = "addChild"
							end if 
       ...
       %>
	level1ID = theMenu.addEntry(level1ID, "Folder", "Tools", "", "")
	<% if userinfo("um_access") then  %>
	level2ID = theMenu.addChild(level1ID, "Document", "Utility Manager", "", "");
    level3ID = theMenu.addChild(level2ID, "Document", "Building Manager", "/genergy2/setup/portfolioedit.asp?pid=<%=userinfo("um_pid")%>", "<%=userinfo("um_target")%>");
		<% if trim(Session("loginemail")) = "realgy" then %>
		level3ID = theMenu.addChild(level2ID, "Document", "Rate Manager", "/genergy2/setup/regionedit.asp?rid=<%=userinfo("um_pid")%>", "<%=userinfo("um_target")%>");
		<%end if %>
	<% 
	end if 
	userinfo.close
	%>
	level2ID = theMenu.addChild(level1ID, "Document", "Manual", "manual/contents.htm", "");
	level2ID = theMenu.addChild(level1ID, "Document", "Downloads", "downloads.htm", "");
...
if (self.name == 'menu') {
	...
	if (floatingMode) {
		...
		self.document.writeln('<frameset cols="100%" rows="30,*" BORDER="0" FRAMEBORDER="0" FRAMESPACING="0" onResize="defaultResizeHandler()"; onunload="">');
		self.document.writeln('<frame name="title" src="banner.htm" FRAMEBORDER="0" scrolling="no" noresize marginwidth="0" marginheight="0">');
		self.document.writeln('<frameset id="sidebar" cols="271,*" rows="100%">');
		...
		self.document.writeln('</frameset>');
		self.document.writeln('<frame name="main" src="' + thePage +'" scrolling="auto">');
		self.document.writeln('</frameset>');
		self.document.writeln('</frameset>');
	}
}
