...
<body bgcolor="ffffff">
    <% 
	sqlstr2 = "select DISTINCT cat.category as category, sum(cat.hours) as hours from  (select employees.category as category, sum(hours) as hours from invoice_submission, [job log], [employees] where jobno=[entry id] and username=matricola and invoice_submission.date> [job log].last_invoice and invoice_submission.jobno='"& job & "' group by employees.category union select invoice_submission.category as category, sum(hours) as hours from invoice_submission, [job log], [employees] where jobno=[entry id] and username=matricola and invoice_submission.date>=[job log].last_invoice and invoice_submission.jobno='" & job & "' group by invoice_submission.category) as cat group by cat.category"
	...
	sqlstr2 = "select sum(hours) as hours, sum(hours_bill) as hours_bill, sum(billable) as billable from invoice_submission, [job log] where jobno=[entry id] and invoice_submission.date>= [job log].last_invoice and invoice_submission.jobno='"&job&"' "
...
<IFRAME name="detail" width="100%" height="150" src="null.htm" scrolling="auto" marginwidth="0" marginheight="0" ></IFRAME>
		<%
			...
			rst1.close
    		strsql="select [entry id] from [job log] where [entry id]='" & job &"'"
