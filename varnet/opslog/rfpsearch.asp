...
if completed="1" then
	...
	sqlstr = "select * from " & "rfplog" & " where " & item & " =" & input & " and [current status] <> 'Proposal Accepted' and [current status] <> 'Proposal Rejected'" 	
	else
	
	sqlstr = "select * from " & "rfplog" & " where " & item & " like '%" & input & "%' and [current status] <> 'Proposal Accepted' and [current status] <> 'Proposal Rejected'" 
	
	end if
		
    if item="customer" and not isempty(var) then
		sqlstr="select distinct rfplog. customer as customer1, customers.companyname, [entry id], ChgOrderRefNum, [% completed], salesmanager, description from rfplog join customers on rfplog.customer=customers.customerid and companyname like '%" & var & "%' and [current status] <> 'Proposal Accepted' and [current status] <> 'Proposal Rejected'"
	end if
