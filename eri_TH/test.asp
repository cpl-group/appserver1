...
sql="SELECT Buildings.Management, SUM(Tenant_info.CCY) AS Tot_Year, SUM(Tenant_info.ccm) " &_
    ...
    "Buildings.BldgNum = Tenant_info.Bldg_no WHERE (Buildings.Include = 1) AND (Buildings.owner_id = N'" & request("ownerid") & "') AND " &_ 
    "(Tenant_info.Lease_Exp_Date > { fn NOW() }) group by buildings.management"
...
<p align="left"><i>Update as&nbsp; <!--webbot bot="Timestamp" s-type="REGENERATED" s-format="%B %d, %Y" startspan -->April 16, 2002<!--webbot bot="Timestamp" endspan i-checksum="17445" --></i></p>
...
sql = "SELECT Buildings.BldgNum, Buildings.Strt, COUNT(Tenant_info.Tenant_no) AS Tot_tenant, " &_ 
    ...
    "Tenant_info ON Buildings.BldgNum = Tenant_info.Bldg_no WHERE (Buildings.Include = 1) AND (Buildings.owner_id = N'" & request("ownerid") & "') AND " &_ 
