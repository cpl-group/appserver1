...
if session("roleid") = 1 then 'is tenant and needs to pull tenant info
    rstM.Open "SELECT LeaseUtilityId FROM tblLeases INNER JOIN tblLeasesUtilityPrices ON tblLeases.BillingId = tblLeasesUtilityPrices.BillingId WHERE tblLeases.TenantNum='"&Request("tenantnum")&"'", cnnM
...
if portfolioid<>"" then
    ...
    IFrame1 = "lmpload2.asp?m="&m&"&d="&d&"&b="&b&"&s="&s&"&e="&e&"&luid="&luid&"&lmp="&lmp&"&i=1&nozoom="&request.querystring("nozoom")
...
<table width="714" border="0" cellspacing="0" cellpadding="0" align="center" height="270">
