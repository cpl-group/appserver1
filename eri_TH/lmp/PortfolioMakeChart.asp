<%Option Explicit
dim date, s, e, pid, summation, cnn, rst, cnn2, rst2, objChart,bldglist, kwhindex
...
sql = "select buildings.bldgnum, meterid from buildings join meters on buildings.bldgnum = meters.bldgnum where portfolioid = '"& pid &"' and pp=1 order by buildings.bldgnum"
...
if not bldglist.EOF then 
	...
	while not bldglist.EOF 
		sql = "SELECT (datepart(hour,date)*100) as time, SUM(kwh) as kwh FROM pulse_"&bldglist("bldgnum")&" WHERE meterid="& bldglist("meterid") &" AND left(convert(char(20),date,101),11) = left(convert(char(20),convert(datetime,'"&date&"',101),101),11) AND ((datepart(hour,date)*100) BETWEEN "& s &" AND "& e &") GROUP BY (datepart(hour,date)*100) ORDER BY (datepart(hour,date)*100) "
'   response.write sql
'		response.end
		rst.open sql, cnn
		...
		while(not(rst.EOF) and index<24)
			title(index) = rst("time")
			AggKW(index) = formatnumber(AggKW(index) + cdbl(rst("kwh")), 0)
			index=index + 1
			rst.moveNext
		wend
		rst.close
	...
	next
		...
		objChart.ChartArea(0).AddChart 6, 1, 1, , 2
