...
<table width="102%" border="1" cellspacing="0" cellpadding="0" height="100%" align="center" bordercolor="#333333">
  <tr>
    <td>
      <div align="center"><%if session("userid")<>"nyserda2" and trim(luid)="" then%><a href="peakDemandPieload2.asp?b=<%=b%>&luid=<%=luid%>&explode=<%=explode%>&byear=<%=byear%>&bperiod=<%=bperiod%>&coor="><%end if%><img border="0" src="peakDemandPie.asp?b=<%=b%>&luid=<%=luid%>&explode=<%=explode%>&byear=<%=byear%>&bperiod=<%=bperiod%>&coor=<%=coor%>" Ismap><%if session("userid")<>"nyserda2" and trim(luid)="" then%></a><%end if%></div>
    </td>
  </tr>
</table>
</body>
