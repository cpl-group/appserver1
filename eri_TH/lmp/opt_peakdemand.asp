...
if rs.state = adstateopen then 
    Do Until rs.EOF
        ...
        if cDBL(rs("percentage"))>0 and trim(luid)="" then
            if session("userid")<>"nyserda2" then response.write "onclick=""parent.lmp.document.location.href='peakdemandpieload2.asp?b="& b &"&byear="& byear &"&bperiod="& bperiod &"&explode=" 'this disables
