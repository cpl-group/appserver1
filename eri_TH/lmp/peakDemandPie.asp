...
if trim(luid)<>"" then
    ...
    if session("userid")="nyserda2" then cmd.CommandText = "sp_peak_contribution_MECH"
...
if rs.state = adstateopen then 
	...
	do while not(rs.EOF)
		if formatnumber(rs("percentage"))>0 then 
			objChart.AddData formatnumber(rs("percentage")),0, rs("labelname")
...
if explode<>"" and explode<>"-1" then
    ...
    rs.open "SELECT meternum,kwh FROM pulse_"&b&" p INNER join meters on p.meterid=meters.meterid WHERE meters.pp=0 and meters.LeaseUtilityId="& dataluid(explode) &" and datediff(minute,[date],'"& dmaxdate &" "& cmd.Parameters("tmax") &"')=0 ORDER BY kwh desc", cnn
    'response.write "SELECT meternum,kwh FROM pulse_readings INNER join meters on pulse_readings.meterid=meters.meterid WHERE meters.LeaseUtilityId="& dataluid(explode) &" and datediff(minute,[date],'"& dmaxdate &" "& cmd.Parameters("tmax") &"')=0 ORDER BY kwh desc"
    'response.end
