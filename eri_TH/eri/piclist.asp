...
openStr= getConnect(0,bldg,"engineering")
cnn1.Open openStr
...
Else
  Response.Write "parent.frames.info.location = " & _
                  Chr(34) & _
                  "info.asp" & _
                  "?qcatnr=" & rst1("Tenant_no") & "&bldg="&bldg&  _
                  Chr(34) & vbCrLf
End If
...
%>
    <tr<% if clng(rst1("lastsurkwh")) > 0 then response.write " bgcolor=""#00CC66"""%><%if isnull(rst1("Move_out_date")) then response.write " bgcolor=""#FFFFCC"""%> bgcolor="#FFFFFF"> 
      <td width="10%" height="20" align="center"><font face="Arial, Helvetica, sans-serif" size="2"> 
        <%
		Response.write "<a href=" & chr(34) & "info.asp?" & "bldg="&bldg&"&qcatnr=" & rst1("Tenant_no") & chr(34) & " target=info>" & rst1("Tenant_no")
%>
        </font></td>
      <td width="30%" height="20" align="center"> 
        <p align="left"><font face="Arial, Helvetica, sans-serif" size="2"><%=rst1("Tenantname")%> 
        </font>
    </td>
