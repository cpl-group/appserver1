...
sql = "SELECT Buildings.BldgNum, Buildings.Strt, COUNT(Tenant_info.Tenant_no) AS Tot_tenant, " &_ 
    ...
    "case when SUM(Tenant_info.sqft)=0 then 0 else SUM(Tenant_info.CCY) / SUM(Tenant_info.sqft) end AS avg_sqft,Sur_sqft=isnull((select sum(tenant_info.sqft)as Sqft FROM tenant_info WHERE last_sur_kw <> 0 and  (Tenant_info.Lease_Exp_Date > GETDATE())and bldg_no =bldgnum),0) FROM Buildings INNER JOIN " &_
    "Tenant_info ON Buildings.BldgNum = Tenant_info.Bldg_no WHERE (Buildings.Owner_id = N'" & request("portfolioid") & "') AND " &_ 
    "(Tenant_info.Lease_Exp_Date > { fn NOW() }) and tenant_info.sqft <> 0 GROUP BY Buildings.BldgNum, Buildings.Strt"
rsCat.Open sql, cnn1, adOpenStatic, adLockReadOnly
...
Note:  This calculation only accounts for tenants with available square footage.
