...
function loadentry(luid, ypid){
	...
	var temp = 'meterbillsummary.asp?l=' +luid+'&Y='+ypid+'&building=<%=server.urlencode(request("b"))%>'
...
if leaseid<>0 then
	title = "Tenant"
	sql="SELECT top 12 TotalAmt,b.BillYear, b.BillPeriod, sum(m.KWHUsed) as KWH, sum(Demand_P) as demand_P, m.ypid, m.leaseutilityid FROM tblmetersbyperiod m INNER JOIN tblbillbyperiod b on b.ypid=m.ypid and b.LeaseUtilityId=m.LeaseUtilityId WHERE meterid in (select meterid from meters where leaseutilityid =" & leaseid & ") and b.bldgnum='" & bldg & "' and b.posted = 1 GROUP BY b.BillYear, b.BillPeriod, m.ypid, m.leaseutilityid, TotalAmt order by b.billyear desc,b.billperiod desc"
else
	title = "Building"
	sql="SELECT top 12 sum(TotalAmt) as TotalAmt, BillYear, BillPeriod, (select sum(kwhused) from tblmetersbyperiod as mbp where bldgnum='"&bldg&"' and mbp.ypid=byp.ypid) as kwh, (select sum(demand_p) from tblmetersbyperiod as mbp where bldgnum='"&bldg&"' and mbp.ypid=byp.ypid) as demand_P, ypid, 0 as leaseutilityid FROM tblbillbyperiod byp WHERE bldgnum='" & bldg & "' and posted = 1 GROUP BY BillYear, BillPeriod, ypid order by byp.billyear desc ,byp.billperiod desc"
end if
'SELECT top 12 sum(TotalAmt) as TotalAmt, BillYear, BillPeriod, (select sum(kwhused) from tblmetersbyperiod as mbp where bldgnum='100wall' and mbp.ypid=byp.ypid) as kwh, sum(Demand) as demand_P, ypid, 0 as leaseutilityid FROM tblbillbyperiod byp WHERE bldgnum='100wall' and posted = 1 GROUP BY BillYear, BillPeriod, ypid order by ypid desc
'response.write sql
'response.end
