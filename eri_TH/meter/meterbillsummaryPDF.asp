...
if leaseid<>0 then
	...
	title = "Building"
	cmd1.CommandText = "select billyear, billperiod, sum(energy) as energy, sum(demand) as demand, avg(adminfee) as adminfee, sum(addonfee) as addonfee, sum(tax) as tax, sum(totalamt) as totalamt from tblbillbyperiod where ypid=" & ypid &" and posted =1 group by billyear, billperiod"
end if
...
function viewbill(ypid,lid) {
			var temp= "../pdfMaker/pdfBatchPrint.asp?y=" + ypid + "&l=" + lid 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#0099FF">
  <tr>
    ...
    <td width="46%" bgcolor="#0099FF" align="right" style="font-family:arial;font-size:12;text-decoration:none;color:#FFFFFF;">
		...
		<%else%>
		<a style="font-family:arial;font-size:12;text-decoration:none;color:#FFFFFF;" href="/eri_th/pdfmaker/pdfbatchprint.asp?building=<%=server.urlencode(building)%>&bperiod=<%=bperiod%>&byear=<%=byear%>" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'white'"><b>Download&nbsp;all&nbsp;bills&nbsp;for&nbsp;this&nbsp;building</b></a>&nbsp;|&nbsp;
		<a style="font-family:arial;font-size:12;text-decoration:none;color:#FFFFFF;" target="_blank" href="/eri_th/pdfMaker/pdfBillSummary.asp?building=<%=server.urlencode(building)%>&byear=<%=byear%>&bperiod=<%=bperiod%>" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'white'"><b>Download&nbsp;Bill&nbsp;Summary&nbsp;Report</b></a>
		<%end if%>
	</td>
  </tr>
</table>
