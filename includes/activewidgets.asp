...
function activewidgets_grid(name, oRecordset, id)

	Dim i, columns, rows, s
	Dim column_count, row_count, val
	...
	Do while (Not oRecordset.eof)
		...
		rows = rows & "["
		For i=0 to (column_count-1)
			if isnull(oRecordset(i)) then 
				val = "NA"
			else
				val = oRecordset(i)
			end if 
			rows = rows & """" & activewidgets_html(val) & """, "
		Next
		rows = rows & "]," & vbNewLine
